...
@Component({
  selector: 'app-latency-items-source',
  templateUrl: './latency-items-source.component.html',
  styleUrls: ['./latency-items-source.component.scss']
})
export class LatencySourceItemsComponent implements OnInit {
  ...
  ngOnInit() {    
    ...
    this.editForm = this.fb.group({      
      ...
      latency: [1000, Validators.required],      
  ...
  getSourceItems(){
    this.sourcesGateway.getSource(this.sourceId).subscribe(source=>{
      ...
      this.sourcesGateway.getLatencySourceItemsByPeriod(this.sourceId, this.startDate, this.endDate).subscribe(data=>{
