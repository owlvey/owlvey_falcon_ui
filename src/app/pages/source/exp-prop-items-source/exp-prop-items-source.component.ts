...
@Component({
  selector: 'app-exp-prop-items-source',
  templateUrl: './exp-prop-items-source.component.html',
  styleUrls: ['./exp-prop-items-source.component.scss']
})
export class ExpPropItemsSourceComponent implements OnInit {
  ...
  getSourceItems(){
    this.sourcesGateway.getSource(this.sourceId).subscribe(source=>{
      ...
      this.sourcesGateway.getExperienceProportionSourceItemsByPeriod(this.sourceId, this.startDate, this.endDate).subscribe(data=>{
  ...
  onSubmit() {    
    ...
    let  defer = this.sourcesGateway.postExperienceSourceItemProportion(model);
