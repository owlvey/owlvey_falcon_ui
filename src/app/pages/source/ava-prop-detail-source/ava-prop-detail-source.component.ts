...
@Component({
  selector: 'app-ava-prop-detail-source',
  templateUrl: './ava-prop-detail-source.component.html',
  styleUrls: ['./ava-prop-detail-source.component.scss']
})
export class AvaPropDetailSourceComponent extends BaseDetailSourceComponent implements OnInit, AfterViewInit {
  ...
  getSource(){    
    this.sourcesGateway.getAvailabilityProportionSourceWithAvailability(this.sourceId, this.startDate, this.endDate).subscribe(data=>{
  ...
  onItemsClick($event){    
    ...
    this.router.navigate(['/pages/sources/availability/proportion/items'], { relativeTo: this.activatedRoute, queryParams: queryParams, queryParamsHandling: 'merge' });             
  }  
