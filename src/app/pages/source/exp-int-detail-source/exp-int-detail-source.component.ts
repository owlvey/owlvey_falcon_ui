...
@Component({
  selector: 'app-exp-int-detail-source',
  templateUrl: './exp-int-detail-source.component.html',
  styleUrls: ['./exp-int-detail-source.component.scss']
})
export class ExpIntDetailSourceComponent  extends BaseDetailSourceComponent implements OnInit, AfterViewInit {
  ...
  getSource(){    
    this.sourcesGateway.getExperienceInteractionSourceWithAvailability(this.sourceId, this.startDate, this.endDate).subscribe(data=>{
  ...
  onItemsClick($event){    
    ...
    this.router.navigate(['/pages/sources/experience/interaction/items'], { relativeTo: this.activatedRoute, queryParams: queryParams, queryParamsHandling: 'merge' });             
  }  
