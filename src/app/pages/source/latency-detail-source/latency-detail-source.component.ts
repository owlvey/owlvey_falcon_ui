...
@Component({
  selector: 'app-latency-detail-source',
  templateUrl: './latency-detail-source.component.html',
  styleUrls: ['./latency-detail-source.component.scss']
})
export class LatencyDetailSourceComponent extends BaseDetailSourceComponent implements OnInit, AfterViewInit {
  ...
  getSource() {
    this.sourcesGateway.getLatencySourceWithAvailability(this.sourceId, this.startDate, this.endDate).subscribe(data => {
  ...
  onItemsClick($event) {
    ...
    this.router.navigate(['/pages/sources/latency/items'], { relativeTo: this.activatedRoute, queryParams: queryParams, queryParamsHandling: 'merge' });
  }
  onEditClick(event) {
    let queryParams: Params = {};
    this.router.navigate(['/pages/sources/latency/edit'], { relativeTo: this.activatedRoute, queryParams: queryParams, queryParamsHandling: 'merge' });
  }
  ngAfterViewInit() {
  }
