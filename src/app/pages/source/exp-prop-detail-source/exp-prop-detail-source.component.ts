...
@Component({
  selector: 'app-exp-prop-detail-source',
  templateUrl: './exp-prop-detail-source.component.html',
  styleUrls: ['./exp-prop-detail-source.component.scss']
})
export class ExpPropDetailSourceComponent extends BaseDetailSourceComponent implements OnInit, AfterViewInit {
  ...
  getSource(){    
    this.sourcesGateway.getExperienceProportionSourceWithAvailability(this.sourceId, this.startDate, this.endDate).subscribe(data=>{
  ...
  onItemsClick($event){    
    ...
    this.router.navigate(['/pages/sources/experience/proportion/items'], { relativeTo: this.activatedRoute, queryParams: queryParams, queryParamsHandling: 'merge' });             
  }  
  
  onEditClick(event){
    let queryParams: Params = { };
    this.router.navigate(['/pages/sources/experience/edit'], { relativeTo: this.activatedRoute, queryParams: queryParams, queryParamsHandling: 'merge' });     
  }
